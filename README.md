...
- 关键词组是「一组常用的检索词条」，与播放列表一样持久化在 SQLite（`keyword_groups` 表），导航栏「关键词组」页管理：
    ...
    - 加词条的输入框带**与搜索页同一套标签建议**（`tagSuggestions()`，按正在写的那一段查词库，未装词库时用内置表）：点一条把**原始标签**填进去（`namespace:raw`），检索写法由 `entryTerm()` 生成。类型选「作者」时查询先收窄到 `artist:` 命名空间，免得 12 条建议名额被别的命名空间占满；类型选「自定义」时不建议。
