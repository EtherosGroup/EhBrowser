...
ehbrowser.db         SQLite：程序标记、画廊缓存、下载任务、阅读进度、用户播放列表、关键词组
```
...
└── services/      业务编排：配置服务、上游装配、画廊服务、账号服务、浏览器登录、下载服务、检索结果缓存、本地库与 zip 读取、播放列表、关键词组、更新检查、收藏、标签翻译词库、日志、客户端状态诊断
...
├── keyword-groups.ts 关键词组的本地状态（读写走服务端）
...
├── tag-search.ts  选中的标签与关键词组词条 -> 搜索框里的关键词、用户输入 -> 送上游的写法（纯函数）
...
└── components/    面板与通用组件
    ...
    ├── KeywordGroupPage.vue     关键词组页：组与词条的增删改、批量删除
...
- 关键词组是「一组常用的检索词条」，与播放列表一样持久化在 SQLite（`keyword_groups` 表），导航栏「关键词组」页管理：
    - 一个组有组名与**有序**词条列表，词条分三类：`tag`（标签）、`author`（画师名）、`custom`（自定义检索片段）。组内按 `(kind, value)` 去重，组数量无上限。
    - 词条变成搜索框文本的规则在 `web/src/tag-search.ts` 的 `entryTerm()`：`tag` 走标签语法 `namespace:"名字"$`，`author` 补上 `artist:` 命名空间（已经写了命名空间的照原样），`custom` 原样插入。整组并进搜索框时按词条去重（`mergeQuery()`）。
    - 管理页（`KeywordGroupPage.vue`）能新建、点组名就地改名、加词条（选类型 + 填值）、点词条上的 × 移除、删组（弹窗二次确认），以及勾选后批量删除。每个写操作都回整份列表（`keywords.*` 几条路由），界面拿到直接替换本地状态。
    - 加词条的输入框带**与搜索页同一套标签建议**（`tagSuggestions()`，按正在写的那一段查词库，未装词库时用内置表）：点一条把**原始标签**填进去（`namespace:raw`），检索写法由 `entryTerm()` 生成。类型选「作者」时查询先收窄到 `artist:` 命名空间，免得 12 条建议名额被别的命名空间占满；类型选「自定义」时不建议。分段规则与搜索页一致（只有空格算分隔），输入框下面那行小字也照写。
    - 入口两处：搜索页关键词输入框右侧的「+」打开选组弹窗，点一组把整组词条并进搜索框（输入框里已有的内容保留，重复词条不重复插入）；画廊详情页多选标签后，浮动区里多了「添加到/新建关键词组」，弹窗里可以新建一个组，或追加进已有组（追加的去重由服务端做）。
...
- 搜索页的输入框下面依次是两块可选区域（都只在有条目时出现），顺序即「关键词 / 标签 / 搜索历史」：
    - 标签建议：按输入框里**正在写的那一段**（最后一个空格之后）在词库里找标签（`web/src/translation-db.ts` 的 `searchTags`，反查索引在装词库时建好，4 万条标签一次查找约 1～4ms）。显示成 `译名（命名空间: 原文）`，每行一个；点一条就把上游的检索写法填进输入框（`namespace:"多词标签"$`，写法与 `tag-search.ts` 一致）。
    ...
    - 写入时只替换正在写的那一段（`[^\s]*$`），前面的标签与分隔符（如 `language:chinese 男`）都留着，写完把光标放回末尾，方便接着加标签。
    - **空格是唯一的分隔**，因此打完一个标签敲一个空格就等于「开始写下一个标签」：建议区立刻换到空格后面那一段，不用先删掉前面的内容。逗号不再有特殊含义（早先是空格与逗号都算分隔），输入框下面那行小字写的就是这条规则。
    - 送上游前仍会把逗号换成空格（`tag-search.ts` 的 `normalizeQuery`）：上游按空格切词、官方搜索规则也写明逗号不是分隔符。留着这一步是为了兼容历史记录里带逗号的旧写法（`f:milf,m:muscle`），新写法直接写 `f:milf m:muscle` 即可。
