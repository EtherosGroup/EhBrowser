...
| 网络    | 中国大陆直连 e-hentai.org 不可达，必须自备代理（HTTP 或 SOCKS5）    |
| 账号    | E-Hentai 账号；访问里站需账号具备 ex 权限                           |

运行时零依赖，`typescript` 与 `oxfmt` 仅用于开发。
...
2. 浏览器打开该地址。
3. 首次使用需在界面中完成两项设置：
    - **代理**：填入可用的 HTTP / SOCKS5 代理地址。未配置代理时所有上游请求都会超时。
...
依赖方向单向：`main → api → services → config → platform`。
